...
RaspberryPi = {
  Parts: {
    ...
    MicroUsbPlug: function(port) {
      ...
      return util
        .group('plug,connector,strainrelief,cord,srcutoutup,srcutoutdown', [
          plug,
          connector,
          strainrelief,
          cord,
  ...
  BPlusMounting: {
    holes: function(mb, options) {
      ...
      var hole = RaspberryPi.Parts.MountingHole(
        options.diameter,
        options.height
      )
        .snap(mb, 'xy', 'inside-')
  ...
  BPlus: function(three) {
    ...
    // Right side parts
    group.add(
      RightSide(this.Parts.EthernetJack(), mb).midlineTo('y', 10.25),
      'ethernet'
    );

    var usb = this.Parts.UsbJack();
    var usbt = util.array.add(
    ...
    group.add(
      usb.clone().translate(usbt, util.calcmidlineTo(usb.parts.body, 'y', 29)),
      'usb1',
      false,
      'usb1'
    );
    ...
    group.add(
      usb.clone().translate(usbt, util.calcmidlineTo(usb.parts.body, 'y', 47)),
      'usb2',
      false,
      'usb2'
    );
  ...
  Hat: function(pi) {
    ...
    var hole = this.Parts.MountingHole().snap(mb, 'xy', 'inside-');
    var holes = union(
  ...
  PiTFT22: function() {
    ...
    var lcd = LeftSide(Parts.Cube([45.97, 34.8, 4]), mb)
      .snap(mb, 'z', 'outside-')
      .midlineTo('x', 33.4)
      .midlineTo('y', 27.18)
      .color('black');
    var lcdbevel = LeftSide(Parts.Cube([55, 40, 3.5]), mb)
      .snap(mb, 'z', 'outside-')
      .translate([8, 6, 0]);
    ...
    var group = util.group(
      'mb,gpio,lcd,lcdbevel,button1,button2,button3,button4',
      [mb, gpio, lcd, lcdbevel, buttons[0], buttons[1], buttons[2], buttons[3]]
    );
  ...
  PiTFT24: function(options, pi) {
    ...
    hat.add(
      lcdbevel.translate(
        ...
        lcdbevel.calcSnap(mb, 'z', 'outside-'),
        [4.5, 7, 0]
      ),
      'lcdbevel'
    );
  ...
  CameraModuleV1: function() {
    ...
    g.add(
      util.group('hole0,hole1,hole2,hole3', g.holes),
      'hole',
      false,
      'holes'
    );
    ...
    g.add(util.group(mounts), 'mounts', true, 'mounts');
    ...
    g.add(util.group(pins), 'pins', true, 'pins');
